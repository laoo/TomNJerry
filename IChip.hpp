...
public:
  ...
  static AdvanceResult readByte( uint32_t addres )
  {
    return AdvanceResult{ kByteFlag | ( (uint64_t)addres << 32 ) & kAddressMask };
  }
  static AdvanceResult readShort( uint32_t addres )
  {
    return AdvanceResult{ kShortFlag | ( (uint64_t)addres << 32 ) & kAddressMask };
  }
  static AdvanceResult readLong( uint32_t addres )
  {
    return AdvanceResult{ kLongFlag | ( (uint64_t)addres << 32 ) & kAddressMask };
  }
